...
#         (B.1.177) and Alpha (B.1.1.7) variants using data in England
#
...
cases      <- example.data.case_cog_summer_alpha()
...
prior_daily_seed_max = c( 0.001, 20, 20)
...
data = list(
  ...
  n_strains = 3L,
  data_freq = 7L,
  cases     = as.matrix( cases[, .(cases_WT, cases_summer, cases_alpha)] ),
...
s1 = subplot( list( p1, p2), nrows = 2, shareX = FALSE, titleX = TRUE, titleY = TRUE, margin = 0.05)
s2 = subplot( list( p3, p4), nrows = 2, shareX = TRUE, shareY = FALSE, titleY = TRUE, margin = 0.05 )
